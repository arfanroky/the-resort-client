...
const AddExperiences = () => {
  ...
  const imageStorageKey = `77150875e4a8eb0161d35b4abe2f812a`;
  ...
  const onSubmit = async (e) => {
    ...
    if (data.success) {
      ...
      await axios
        .post('http://localhost:5000/experiences', postExperiences)
        .then((res) => {
          if (res.data.success) {
  ...
  return (
    <div className="flex items-center h-screen justify-center">
      <form className="md:w-[400px] mx-auto" onSubmit={handleSubmit(onSubmit)}>
        <h1 className="text-center mb-12 text-5xl font-bold">Experiences</h1>
        <input
